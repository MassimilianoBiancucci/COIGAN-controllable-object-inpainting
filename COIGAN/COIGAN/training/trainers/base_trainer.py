...
class BaseObjectInpaintingTrainingModule(ptl.LightningModule):
    ...
    def _do_step(self, batch, batch_idx, mode='train', optimizer_idx=None, extra_val_key=None):
        """
        Execute a training step for the given batch.

        Args:
            batch: batch of data
            batch_idx: index of the batch
            mode: 'train' or 'val'
            optimizer_idx: index that specify which model must be trained in this step
        """
        ...
        # run an optimization step for generator
        if optimizer_idx is None or optimizer_idx == 0:  # step for generator
            total_loss, metrics = self.generator_loss(batch)

        # run an optimization step for discriminator
        elif optimizer_idx is None or optimizer_idx == 1:  # step for discriminator
            if self.config.losses.adversarial.weight > 0:
                total_loss, metrics = self.discriminator_loss(batch)

        #if in the main process and it's time to visualize or in test mode
    ...
    def get_ddp_rank(self):
        """Get current DDP rank or None if not in DDP mode"""

...
def make_r1_gp(discr_real_pred, real_batch):
    ...
    Args:
        discr_real_pred: Tensor, discriminator output for real_batch
        real_batch: Tensor, a batch of real samples
...
class NonSaturatingWithR1(BaseAdversarialLoss):
    """
    TODO: interesting implementation but need a complete refactor to match the object impainting task.
    """
    def __init__(self, gp_coef=5, weight=1, mask_as_fake_target=False, allow_scale_mask=False,
        ...
        """
        Non-saturating GAN with R1 gradient penalty

        Args:
            gp_coef: float, coefficient for R1 gradient penalty
            weight: float, weight of this loss
            mask_as_fake_target: bool, if True, then mask is treated as fake target
            allow_scale_mask: bool, if True, then mask is allowed to be scaled to match the size of the image
            mask_scale_mode: str, mode for scaling mask
            extra_mask_weight_for_gen: float, extra weight for generator loss if mask_as_fake_target is True
            use_unmasked_for_gen: bool, if True, then generator is trained on unmasked regions
            use_unmasked_for_discr: bool, if True, then discriminator is trained on unmasked regions
        """
    ...
    def generator_loss(self, real_batch: torch.Tensor, fake_batch: torch.Tensor,
        ...
        """
        Calculate generator loss and call .backward() on it

        Args:
            real_batch: Tensor, a batch of real samples
            fake_batch: Tensor, a batch of samples produced by generator
            discr_real_pred: Tensor, discriminator output for real_batch
            discr_fake_pred: Tensor, discriminator output for fake_batch
            mask: Tensor, actual mask, which was at input of generator when making fake_batch
        
        Returns:
            total generator loss along with some values that might be interesting to log
        """
    ...
    def pre_discriminator_step(self, real_batch: torch.Tensor, fake_batch: torch.Tensor,
                               generator: nn.Module, discriminator: nn.Module):
        """
        Do some operations before discriminator step
    ...
    def discriminator_loss(self, real_batch: torch.Tensor, fake_batch: torch.Tensor,
        ...
        Returns:
            total discriminator loss along with some values that might be interesting to log
        """
...
class BCELoss(BaseAdversarialLoss):
    ...
    def __init__(self, weight):
        """
        Args:
            weight: float, weight of generator loss
        """
        self.weight = weight
...
def make_discrim_loss(kind, **kwargs):
    ...
    Args:
        kind: str, discriminator loss kind
        kwargs: dict, discriminator loss kwargs
    """
    if kind == 'r1':

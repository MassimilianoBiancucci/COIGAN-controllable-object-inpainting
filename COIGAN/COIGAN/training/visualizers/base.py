...
def visualize_mask_and_images_batch(batch: Dict[str, torch.Tensor], keys: List[str], max_items=10,
    ...
    Args:
        batch (Dict[str, torch.Tensor]): batch of images and masks
        keys (List[str]): list of keys to visualize
        max_items (int, optional): number of elements to use in the batch. Defaults to 10.
        last_without_mask (bool, optional): _description_. Defaults to True.
        rescale_keys (_type_, optional): _description_. Defaults to None.
